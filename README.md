...
<p align="center">
  在终端、Web 和 Windows 桌面悬浮窗里，统一查看多个 AI Provider 的账号额度，并按需写入 IDE 与 Agent Harness。
</p>
...
- **额度看板**：终端、Web、悬浮窗三端共用同一份结果，不必来回打开多个客户端。
...
- **三端可查**：终端适合脚本与 Agent，Web 适合完整管理，Windows 悬浮窗适合桌面常驻。
- **自动发现**：读取 Codex、OpenCode、Cockpit、Grok CLI、Claude Code、Cursor 等本机登录态，也支持环境变量、JSON 和手动添加。
- **共享快照**：CLI、Web、悬浮窗共用 `~/.quota-cli/quota-snapshot.json`，跨进程锁会合并同一刷新周期的请求。
...
| --- | --- | --- | --- | --- |
| 终端 | `quota show` | SSH、脚本、Agent、快速查看 | ANSI 额度看板、单次查询、持续刷新、强制联网 | Windows / macOS / Linux |
| Web | `quota ui` | 账号与额度的完整管理 | Provider 筛选、四款主题、添加账号、OAuth、历史恢复、写入 Harness、OpenAI 重置额度 | 默认仅 `127.0.0.1:18765` |
| 悬浮窗 | `quota float` | 桌面常驻、随时扫一眼 | 置顶、拖动、缩放、透明度、托盘、手动刷新、展示项选择 | 完整体验以 Windows 为准 |

三个入口连接的是同一份本地额度快照：一个界面完成刷新后，另外两个界面会复用结果，避免同一周期重复请求 Provider。

<p align="center">
  <img src=".image/terminal-dashboard.png" width="43%" alt="Quota CLI 终端额度看板">
  &nbsp;&nbsp;
...
先运行一次 `quota show --once` 收集账号，再选择一种入口：

- 终端：运行 `quota`，选择 `8) 同步到 harness`。
- Web：运行 `quota ui`，在账号卡片点击“写入到…”。
...
| --- | --- |
| `quota` | 打开交互菜单 |
| `quota show` | 按配置的间隔持续刷新；默认 15 秒 |
| `quota show --watch 60` | 每 60 秒刷新 |
| `quota show --once` | 查询一次后退出，适合脚本与 Agent |
| `quota show --once --force` | 忽略共享快照，立即发起一次联网刷新 |
...
3. **共享结果**：结果写入不含密钥的共享快照，终端、Web、悬浮窗共同读取。
...
| `quota-snapshot.json` | 三端共用的展示快照 | 否 |
...
- Web 和终端只展示脱敏后的 Key；共享快照不会写入 access token、refresh token 或 API Key。
...
因此，不要把 Web UI 直接部署成公开服务。服务器场景建议通过 SSH 使用 `quota show --once`，凭证与数据仍放在运行用户自己的数据目录中。
...
.\.venv\Scripts\python quota.py show --once
.\.venv\Scripts\python -m unittest discover -s tests -v

# macOS / Linux
./.venv/bin/python -m pip install -r requirements.txt
./.venv/bin/python quota.py --help
./.venv/bin/python quota.py show --once
...
Agent 查询额度时建议使用一次性命令：

```bash
quota show --once
```

只有用户明确要求立即联网时，才使用 `quota show --once --force`；不要在日志或回复中输出完整凭证。
