...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Release-v0.6.5-C3B191" alt="Release v0.6.5">
...
- **Claude 错误暂停**：额度或账号信息查询失败后暂停该账号的自动请求，手动刷新才再尝试一次；网络错误、`429` 和服务错误不立即重试，`401` 最多续期一次。本地 Claude Code 登录态会读取 refresh token 与过期时间，续期成功后回写原文件；凭据失效时需在 Claude Code 重新登录后手动刷新。
